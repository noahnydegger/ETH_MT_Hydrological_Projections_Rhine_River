...
plot_monthly_with_yearly_boxplots <- function(monthly_data, yearly_data, variable_name, stat, daily_value, y_label) {
  ...
  p <- ggplot(monthly_data, aes_string(x = "MonthAbb", y = variable_name, fill = "Source")) +
    ...
    labs(
      title = paste("Monthly and Yearly", stat, "of Daily", daily_value, "Temperature"),
      x = "Time Period",
      y = y_label,
      fill = "Dataset"
    ) +
  ...
  save_dir <- file.path(here::here(), "Plots", "Temperature")
...
plot_monthly_with_yearly_boxplots(combined_months, combined_years, "MeanAvg", "Mean", "avg.", "Temperature [°C]")
plot_monthly_with_yearly_boxplots(combined_months, combined_years, "MeanMax", "Mean", "max", "Temperature [°C]")
plot_monthly_with_yearly_boxplots(combined_months, combined_years, "MeanMin", "Mean", "min", "Temperature [°C]")
plot_monthly_with_yearly_boxplots(combined_months, combined_years, "MeanStd", "Mean", "std.", "Temperature [°C]")
