...
compute_annual_or_seasonal_knmi <- function(dt, value_col, group_cols, 
...
compute_relative_mean_knmi <- function(dt_mean, value_col, comparison_col, comparison_ref, 
...
compute_member_differences_knmi <- function(dt, value_col, group_cols, linking_cols, comparison_col, comparison_ref,
                                       statistic = "mean", seasonal = FALSE, monthly = FALSE, half_year = FALSE) {
...
plot_annual_horizon_mean_diff_ch2018 <- function(knmi_dt, ch2018_dt, plot_dir, bsn, color_col, color_col_levels, line_col, line_col_levels, comparison_ref, value_col, group_cols, stat, info_text = "", y_lim = NULL, rel = FALSE, abs = FALSE) {
  ...
  dt[, x_group_all := paste(get(color_col), dataset, sep = "_")]
  # Construct master ordered reference based on color_col_levels and dataset order
  # This ensures color_col_levels define the base order, and datasets are nested within
  ref_levels <- c("L_KNMI", "M_KNMI", "H_KNMI", 
                  "L_CH2018", "M_CH2018", "H_CH2018")
  
  # Now assign x_facet using match, preserving this order
  ...
  # Save the plot
  save_dir <- file.path(plot_dir, "ch2018_annual_horizon_mean_se", value_col)
  filename <- paste0("ch2018_annual_horizon_", bsn, "_", value_col, info_text, ".pdf")
  save_plot(p, save_dir, filename, width = 18, height = 6)
